...
项目主要使用 Python 完成数据清洗、探索性分析、统计建模、机器学习建模与可视化展示，并最终形成完整的建模结果、图表和论文。
...
基于用户对智能手机不同维度的评价数据，分析以下因素对总体评分的影响：
...
在该部分中，项目使用多元线性回归、岭回归、随机森林等方法进行建模，并对不同模型的预测效果和特征影响程度进行比较分析。

### Q2 定价策略与销量预测

基于产品评分、销售价格、竞品价格、市场规模等变量，建立销量预测模型，并进一步分析不同定价策略下的销量和利润变化，为产品定价提供数据支持。

### Q3 产品设计优化

综合用户评分模型与销量利润模型，对不同产品参数组合进行模拟，筛选利润表现较优的产品设计方案，为产品设计与市场定价提供决策参考。
...
│   ├── 问题1输出结果/          # 问题一：用户评分影响因素分析结果
│   ├── 问题2输出结果/          # 问题二：定价策略与销量预测结果
...
用户评价数据主要包含用户对智能手机不同维度的评分信息，包括总体评分以及外观、屏幕、摄像、续航、性能、发热控制等指标。该数据主要用于分析不同产品设计因素对用户总体评价的影响。

### 市场销售数据

市场销售数据主要包含产品价格、竞品价格、市场规模、销量等变量。该数据主要用于建立销量预测模型，并进一步进行利润分析和定价策略比较。
...
本项目来源于数学建模实践，围绕智能手机产品设计与定价决策展开。通过对用户评价数据和市场销售数据进行分析，尝试从数据角度理解用户偏好、市场响应与企业利润之间的关系。

---

## 个人工作

本人主要参与了项目中的代码实现、数据分析与结果整理工作，具体包括：
